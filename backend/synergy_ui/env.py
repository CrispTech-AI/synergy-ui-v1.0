...
# SYNERGY_UI_DIR should be the directory where env.py resides (SYNERGY_UI/)
SYNERGY_UI_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of SYNERGY_UI_DIR (backend/)
BACKEND_DIR = SYNERGY_UI_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (synergy-ui-dev/)
...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "Synergy UI")
if WEBUI_NAME != "Synergy UI":
    WEBUI_NAME += " (Synergy UI)"

WEBUI_FAVICON_URL = "https://synergyui.com/favicon.png"
...
if FROM_INIT_PY:
    PACKAGE_DATA = {"version": importlib.metadata.version("synergy-ui")}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data("SYNERGY_UI", "CHANGELOG.md") or b"").decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv("DATA_DIR", SYNERGY_UI_DIR / "data")).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "SYNERGY_UI_data", "zip", DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv("DATA_DIR", SYNERGY_UI_DIR / "data"))

STATIC_DIR = Path(os.getenv("STATIC_DIR", SYNERGY_UI_DIR / "static"))

FONTS_DIR = Path(os.getenv("FONTS_DIR", SYNERGY_UI_DIR / "static" / "fonts"))

FRONTEND_BUILD_DIR = Path(os.getenv("FRONTEND_BUILD_DIR", BASE_DIR / "build")).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(
        os.getenv("FRONTEND_BUILD_DIR", SYNERGY_UI_DIR / "frontend")
    ).resolve()
...
REDIS_KEY_PREFIX = os.environ.get("REDIS_KEY_PREFIX", "synergy-ui")
...
OTEL_SERVICE_NAME = os.environ.get("OTEL_SERVICE_NAME", "synergy-ui")
