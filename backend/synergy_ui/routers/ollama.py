    ...
    try:
        ...
        if ENABLE_FORWARD_USER_INFO_HEADERS and user:
            ...
            if metadata and metadata.get("chat_id"):
                headers["X-synergyui-Chat-Id"] = metadata.get("chat_id")
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"Synergy UI: Server Connection Error",
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else "Synergy UI: Server Connection Error",
        )
    finally:
        if not stream:
            await cleanup_response(r, session)
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Synergy UI: Server Connection Error"
            )
        except Exception as e:
    ...
    if url_idx is None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else "Synergy UI: Server Connection Error",
            )
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "Synergy UI: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
