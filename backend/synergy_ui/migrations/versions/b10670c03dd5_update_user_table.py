...
def _convert_column_to_json(table: str, column: str):
    ...
    # SQLite cannot ALTER COLUMN â†’ must recreate column
    if dialect == "sqlite":
        ...
        # 4. Rename new JSON column â†’ original name
...
def upgrade() -> None:
    ...
    # Convert info (TEXT/JSONField) â†’ JSON
    _convert_column_to_json("user", "info")
    # Convert settings (TEXT/JSONField) â†’ JSON
